class ArbolAVL {
  ...
  void search_above_prom_year(Nodo nodo, int index) {
    if (nodo != null) {
      search_above_prom_year(nodo.left, index);
      if ((float)acum_temp_year/cantidad_nodos < nodo.temperatures.get(index)) {
        nodos_lower_prom.add(nodo);
      }
      search_above_prom_year(nodo.right, index);
    }
  }

  //  TEMPERATURA EN UN AÑO MENOR AL PROMEDIO DE TODOS LOS AÑOS
  void search_lower_prom_years(Nodo nodo, int index) {
    if (nodo != null) {
      search_lower_prom_years(nodo.left, index);
      if ((float)acum_temp_years/cantidad_nodos < nodo.temperatures.get(index)) {
        nodos_above_prom.add(nodo);
      }
      search_lower_prom_years(nodo.right, index);
    }
  }
